...
class SynthesisActionResult(TypedDict):
    """Stable envelope for legacy Qt/TCP adapter actions."""
...
class SynthesisStation:
    ...
    def _ensure_deck(self) -> Any:
        deck = self.deck
        if deck is None:
            from yb_sse_devices.resources.synthesis_station_deck.resource import SynthesisStationDeck
    ...
    def _status_snapshot(self) -> SynthesisActionResult:
        return {
            "icon": DECK_ICON,
            "status": self.status,
    ...
    def _require_ok(self, response: dict[str, Any]) -> SynthesisActionResult:
        try:
    ...
    @staticmethod
    def _with_task_id(response: dict[str, Any], task_id: str) -> SynthesisActionResult:
        payload = dict(response)
        payload["task_id"] = task_id
        return payload

    @staticmethod
    def _with_post_id(response: dict[str, Any], post_id: str) -> SynthesisActionResult:
        payload = dict(response)
    ...
    def _task_query(self, action_name: str, task_id: str) -> SynthesisActionResult:
        resolved = self._resolve_task_id(task_id)
    ...
    def _exchange(self, request: dict[str, Any]) -> SynthesisActionResult:
        sock = self._connect()
        encoded = json.dumps(
    ...
    def query_tasks(self, from_id: str = "", only_running: bool = False) -> SynthesisActionResult:
        """查询 TASK 任务队列。
    ...
    def query_posts(self, from_id: str = "", only_running: bool = False) -> SynthesisActionResult:
        """查询 POST 后处理任务队列。
    ...
    def station_status(self) -> SynthesisActionResult:
    ...
    def query_lot(self, lot_id: str) -> SynthesisActionResult:
    ...
    def query_bottle_code(self, bottle_code: str) -> SynthesisActionResult:
        """按装瓶二维码查询对应 LOT 与小坩埚。
    ...
    def test_connection(self) -> SynthesisActionResult:
        try:
            response = self._request("station_status", retry_read=True)
    ...
    def upload_cubic(self, task_id: str = "", fetch_cubic_source: int = 1) -> SynthesisActionResult:
        """上坩埚。
    ...
    def query_upload_cubic_status(self, task_id: str = "") -> SynthesisActionResult:
        """查询上坩埚状态。
    ...
    def close_cabin_outer_door(self, task_id: str = "") -> SynthesisActionResult:
        """方舱人工上料完成后关外舱门。
    ...
    def start_recipt(self, task_id: str = "") -> SynthesisActionResult:
    ...
    def get_recipt_status(self, task_id: str = "") -> SynthesisActionResult:
        """查询各槽位加样状态。
    ...
    def start_acoustic_resonance(self, task_id: str = "") -> SynthesisActionResult:
        """启动声共振；成功启动即返回，不等待结束。
    ...
    def get_acoustic_resonance_status(self, task_id: str = "") -> SynthesisActionResult:
        """查询声共振上料、下料与运行状态。
    ...
    def fetch_acoustic_resonance(self, task_id: str = "") -> SynthesisActionResult:
        """声共振下料。
    ...
    def finish_acoustic_resonance(self, task_id: str = "") -> SynthesisActionResult:
        """通知系统声共振全部结束。
    ...
    def scan_big_cubic_to_bottle(self, task_id: str = "", qrcode: str = "") -> SynthesisActionResult:
        """按大坩埚二维码扫码装瓶。
    ...
    def get_sintering_status(self, post_id: str = "") -> SynthesisActionResult:
        """查询马弗炉与焦耳热烧结状态。
    ...
    def fetch_joule_heating(self, post_id: str = "") -> SynthesisActionResult:
        """焦耳热下坩埚。
    ...
    def fetch_furnace(self, post_id: str = "", furnace_id: int = 1) -> SynthesisActionResult:
        """马弗炉下坩埚。
