...
from yb_sse_devices.common.protocol import (
    DEVICE_KEYS,
    HEALTH_LABELS,
...
class ConductivityActionResult(TypedDict):
    """Stable envelope for legacy TCP actions not used as workflow nodes."""
...
class ConductivityStation:
    ...
    def _ensure_deck(self) -> Any:
        deck = self.deck
        if deck is None:
            from yb_sse_devices.resources.conductivity_station_deck.resource import ConductivityStationDeck
    ...
    def _require_ok(self, response: dict[str, Any]) -> ConductivityActionResult:
        try:
    ...
    def _exchange(self, request: dict[str, Any]) -> ConductivityActionResult:
        sock = self._connect()
        encoded = json.dumps(
    ...
    def _cached_query(self, cache_key: str, action_name: str) -> ConductivityActionResult | None:
        now = time.monotonic()
    ...
    def _sync_deck_occupancy(self, materials: dict[str, list[int]]) -> None:
        from yb_sse_devices.resources.conductivity_funnel.resource import ConductivityFunnel
        from yb_sse_devices.resources.conductivity_mold.resource import ConductivityMold
        from yb_sse_devices.resources.conductivity_sintering_bottle.resource import ConductivitySinteringBottle
    ...
    def _refresh_batch(self) -> ConductivityActionResult:
    ...
    def batch_status(self) -> ConductivityActionResult:
        return self._request("batch_status", retry_read=True)

    @action(always_free=True, description="按批次号查询样品测试结果；空则查当前批次")
    def batch_result(self, batch_id: str = "") -> ConductivityActionResult:
    ...
    def query_batch(self, month: str = "") -> ConductivityActionResult:
    ...
    def test_connection(self) -> ConductivityActionResult:
        try:
    ...
    def stop_current_batch(self) -> ConductivityActionResult:
        response = self._require_ok(self._request("stop_current_batch"))
        self._invalidate_batch_cache()
        return response

    @not_action
    def manual_run(self, step: int) -> ConductivityActionResult:
